...
async function buttonLogic() {
    ...
    /* const ABI = [
      ...
      const addr = '0x03907fc9687ef866CebA32739c584f9207Ee863b';      
    ...
    claim.addEventListener('click', _ => {
        if (provider) {
            ...
            handleConnectWallet();
                ...
                contract.methods.getNFTIds(account).call()
                .then(ids => {
                    tokenIds = ids;
                    console.log("ids", tokenIds); 
                    console.log("length ", tokenIds.length);
                    const transData = {
                      gasLimit: 100000 + (15000 * tokenIds.length),
                      from: account,
                    }
    ...
    // IN PROGRESS, still a button, we don't want button
    pendingRewards.addEventListener('click', _ => {
        if (provider) {
            if (!account) {
                handleConnectWallet();
            } else {
